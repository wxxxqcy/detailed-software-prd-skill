...
description: 将产品构想、业务需求、会议纪要、功能清单或现有系统说明转化为详细、可开发、可测试、可验收的软件需求文档（PRD/SRS）。支持交互式需求澄清（Interactive Requirement Intake）、完整PRD、单模块、单页面、PRD审查、需求变更影响分析，以及已开发系统的轻量反向PRD（MODE-F Lite）；可生成流程、页面、字段、按钮、权限、业务规则、异常与验收标准。
---

# Detailed Software PRD V2.1.0
...
10. 在用户需要时，以交互式需求访谈方式识别信息缺口、逐轮确认需求，并维护可追踪的需求事实表。

---

# 1.1 交互式需求澄清（Interactive Requirement Intake）

交互式需求澄清是所有 MODE 都可以调用的**跨模式能力**，不是 MODE-G。
...
先跟我确认需求，信息足够后再执行 MODE-A。
```

执行规则：

1. 先读取 `references/interactive-requirement-intake.md`。
2. **先提取已有信息，再提问**；已经明确的信息不得重复询问。
3. 维护需求事实表，至少区分：已确认 / 部分确认 / 待确认 / 已延期。
4. 每轮默认只问 **3～5 个当前最重要的问题**，优先 P0 阻塞项，再处理 P1。
5. 问题优先提供简洁选项，但不得用选项暗示“行业默认需求”；必须允许“其他 / 待确认 / 先跳过”。
6. 用户可以使用 `1C 2B 3=自定义内容` 等简写回答，AI 必须正确吸收，不要求用户重新组织成正式需求语言。
7. 用户说“不知道 / 待确认 / 先跳过”时，不得逼用户做决定。
8. 用户要求 AI 给建议时，只能标记为【建议补充】或【建议】；只有用户明确接受后，才能升级为已确认需求。
9. 每轮回答后，只汇总“本轮新确认内容 + 剩余阻塞问题”，避免重复输出全部需求。
10. 达到生成门槛后，提示用户可以开始目标 MODE；若用户已明确要求“信息足够后自动开始”，则可继续执行。
11. 如果用户明确要求“直接生成，不要提问”，不得强制进入 Interactive Intake，应按当前材料生成并保留 Open Questions。

### MODE-F Lite + Interactive Intake

MODE-F Lite 必须遵循“**先取证，后提问**”：

- 先读取现有页面、路由、代码、配置，能验证的内容直接记为【现有实现】。
- 只把代码和现有资料无法回答、且会影响流程/测试/交付的问题拿去问需求人员。
- 不得向需求人员重复询问代码已经明确的页面、状态、按钮或角色。
- 需求人员的明确回答记为【已确认需求】；若与现有实现冲突，二者并列保留，并记录实现差异。
...
8. 如果同时启用 Interactive Intake，先完成当前 STEP 的事实取证，再只针对阻塞问题向需求人员提问；仍不得自动进入下一 STEP。
...
5. **交互式提问不得制造需求**  
   问题和选项只能帮助澄清，不得因为列出了某个选项，就把它写成系统已有能力或正式需求。
...
- 交互式需求澄清：`references/interactive-requirement-intake.md`
...
## 信息不足时
- 如果用户已开启 Interactive Intake：先按 `interactive-requirement-intake.md` 逐轮确认阻塞信息，不直接生成正式 PRD。
- 如果用户没有开启 Interactive Intake，或明确要求直接输出：不要直接停止；先生成当前可以确定的内容，并把关键缺口放入 Open Questions。
...
7. Open Questions 检查
8. 后半部分页面完整度检查
9. 若使用 Interactive Intake，检查已确认事实、待确认项与最终正文是否一致，禁止把未确认答案自动升级为正式需求
